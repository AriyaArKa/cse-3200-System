...
def _ocr_with_ollama(
    ...
    try:
        ...
        prompt = "Extract all text from this image. Preserve Bangla (বাংলা) characters exactly. Return only the text."
...
def ocr_page_with_gemini(
    ...
    # Try Gemini first (if enabled and not known to be unavailable)
    if config.GEMINI_ENABLED and _service_status.get("gemini_available") is not False:
    ...
    if result is None and config.OLLAMA_ENABLED:
        ...
        if _service_status.get("ollama_available") and _service_status.get(
            ...
            logger.info(
                f"Gemini unavailable, falling back to Ollama ({_service_status['ollama_model']})"
            )
