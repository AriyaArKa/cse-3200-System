...
_BANGLA_CORRECTIONS = {
    ...
    "\u09c7\u09c7": "\u09c7",  # double e-kaar
    "\u09cb\u09cb": "\u09cb",  # double o-kaar
    # Digit confusion pairs
    "O": "0",  # only applied in numeric contexts
    "l": "1",  # only applied in numeric contexts
    "S": "5",  # only applied in numeric contexts
}
...
# A small set of very common Bangla words for quick validation.
# In production this would be loaded from a corpus file.
_COMMON_BANGLA_WORDS = {
    "এবং",
    "তার",
    "এই",
    "একটি",
...
def validate_bangla_with_gemini(text: str) -> Tuple[str, bool]:
    ...
    Only called when word validity is low (< 0.3).
    """
...
def correct_bangla_text(text: str) -> Tuple[str, dict]:
    ...
    if bn_ratio > 0.2 and validity < 0.3 and len(text) > 30:
