...
from .. import config
from ..exceptions import PDFReadError, PageRenderError
...
def open_pdf(pdf_path: str | Path) -> fitz.Document:
    ...
    try:
        ...
        raise PDFReadError(f"Failed to open PDF: {pdf_path}") from exc
...
def render_page_to_image(page: fitz.Page, dpi: int = None) -> bytes:
    ...
    try:
        ...
        raise PageRenderError("Failed to render page to image") from exc
