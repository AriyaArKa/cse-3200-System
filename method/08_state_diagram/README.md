...
A **State Diagram** (also called State Machine Diagram) shows the **different states** an object or entity can be in throughout its lifecycle, and the **transitions (events)** that cause it to move from one state to another. It answers: *"What stages does this entity go through?"*
