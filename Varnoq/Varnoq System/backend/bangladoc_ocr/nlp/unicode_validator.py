...
def devanagari_char_ratio(text: str) -> float:
    ...
    if not alpha:
        return 0.0
    dev_count = sum(1 for ch in alpha if _DEVANAGARI_START <= ord(ch) <= _DEVANAGARI_END)
    return dev_count / len(alpha)


def is_wrong_script_for_bangla(
    text: str,
    min_bangla_ratio: float = 0.10,
    devanagari_reject_threshold: float = 0.16,
) -> bool:
    """True if OCR output looks Devanagari-heavy for a Bangla pipeline."""
    if not text:
        return False
    alpha = [ch for ch in text if ch.isalpha()]
    if len(alpha) < 12:
        return False
    return (
        devanagari_char_ratio(text) >= devanagari_reject_threshold
        and bangla_char_ratio(text) < min_bangla_ratio
    )


def strip_devanagari_dominant_lines(text: str) -> tuple[str, bool]:
    """Remove lines that are strongly Devanagari-dominant and not Bangla."""
    if not text:
        return text, False
    ...
    for line in text.splitlines():
        ...
        if is_wrong_script_for_bangla(
            line_text,
            min_bangla_ratio=0.10,
            devanagari_reject_threshold=0.35,
        ):
            changed = True
            continue
        kept.append(line)
